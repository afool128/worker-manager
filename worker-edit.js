(() => {
  ...
  function enhanceProfile(){
    ...
    let extra=document.getElementById('profileSalaryExtra');
    if(extra&&extra.dataset.key===key)return;
    if(!extra){extra=document.createElement('div');extra.id='profileSalaryExtra';extra.className='worker';extra.style.marginTop='10px';box.parentNode.insertBefore(extra,box.nextSibling)}
    extra.dataset.key=key;
    extra.innerHTML=`<div class="row"><b class="grow">💰 ${document.getElementById('mo')?.selectedOptions[0]?.text||''} ${s.z.y} Salary</b><button type="button" id="changeSalary">✏️ Salary</button></div><div class="small" style="margin-top:7px">Fixed monthly salary: <b>₹${money(s.salary)}</b></div><div class="small" style="margin-top:5px">Absent: ${s.absent} • Half days: ${s.half} • Deduction: <b>₹${money(s.deduction)}</b></div><div class="row" style="margin-top:8px"><b>Salary earned: ₹${money(s.earned)}</b><span class="grow"></span><b>Paid: ₹${money(s.paid)}</b></div><div style="margin-top:5px"><b>Remaining balance: ₹${money(s.balance)}</b></div><hr style="border:0;border-top:1px solid #eee;margin:12px 0"><div class="row"><b class="grow">Daily Wage</b><button type="button" id="changeDaily">✏️ Change</button></div><div style="margin-top:6px;font-size:18px"><b>₹${money(w.rate)}</b> <span class="small">per day</span></div>`;
    document.getElementById('changeSalary').onclick=()=>editSalary(w.id);
    document.getElementById('changeDaily').onclick=()=>editDaily(w.id);
  }
  let scheduled=false;
